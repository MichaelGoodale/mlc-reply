...
class Grammar:
    ...
    def __apply_helper(self, s):
        ...
        for idx, o in enumerate(out):
            if (
                to_interpet(o) and self.count_recursion < self.max_recursion
            ):  # non-terminals
...
class Rule:
    ...
    def set_primitives(self, list_prims):
        ...
        self.list_prims = list_prims
        self.prim_regexp = (
            "(" + "|".join(self.list_prims) + ")"
        )  # define acceptable string for a primitive

        # get list of all variables in LHS (and for valid rules, the same vars should be in the RHS)
        self.vars = [
            v for v in self.LHS_list if is_prim_var(v) or is_var(v)
        ]  # LHS vars
        rhs_vars = [
            int_strip(v) for v in self.RHS_list if is_prim_var(v) or is_var(v)
        ]  # RHS vars
        self.valid_var_match = all(
            [v in self.vars for v in rhs_vars]
        )  # check that RHS variables are in LHS
...
def get_grammar_miniscan():
    ...
    return str_to_grammar(
        S_miniscan, ["1", "2", "3", "DAX", "thrice", "surround", "after"]
    )
