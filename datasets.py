...
from interpret_grammar import (
    str_to_grammar,
    Grammar,
    Rule,
    get_grammar_miniscan,
    is_prim_var,
    is_var,
)
...
def get_dataset(episode_type):
    ...
    if episode_type == "retrieve":  # BIML (copy only) training
        ...
        D_train = DataAlgAndBias(
            "train", mydir="data_algebraic", p_noise=0.01, min_ns=14
        )
        D_val = DataAlg("val", mydir="data_algebraic")
    elif (
        episode_type == "few_shot_gold"
    ):  # For evaluating few-shot learning task with gold outputs
        D_train = []
        D_val = DataHumanFewShot("gold", inc_support_in_query=True, do_remap=True)
    elif (
        episode_type == "few_shot_human"
    ):  # For evaluating predictions of human behavior on few-shot learning task
        D_train = []
        D_val = DataHumanFewShot(
            "behavior", data_mult=1, inc_support_in_query=False, do_remap=True
        )
    elif (
        episode_type == "few_shot_human_mult10"
    ):  # same dataset as above copied 10 times
        D_train = []
        D_val = DataHumanFewShot(
            "behavior", data_mult=10, inc_support_in_query=False, do_remap=True
        )
    elif (
        episode_type == "open_end_human_all"
    ):  # complete set of human open-ended responses
        D_train = DataHumanOpenEnded(
            "train", inc_support_in_query=True, mydir="data_human/open_ended_all"
        )
        D_val = DataHumanOpenEndedIterative("val", "data_human/open_ended_all")
    elif (
        episode_type == "open_end_freeform"
    ):  # for evaluating model productions on open-ended task
        D_train = []
        D_val = DataFreeformOpenEnded(mydir="data_human/open_ended_all/mock")
    elif (
        episode_type == "probe_human"
    ):  # for evaluating predictions of human behavior on "additional biases" probe tasks (supplement)
        D_train = []
        D_val = DataHumanProbe("val", data_mult=10)
    elif (
        episode_type == "probe_human_w_pool"
    ):  # for training BIML (within-sample) on open-ended task
        ...
        episode_type == "human_vanilla"
    ):  # Basic seq2seq for evaluating predictions of human behavio
        D_train = []
        D_val = DataHumanVanilla(
            "behavior", data_mult=1, inc_support_in_query=False, do_remap=False
        )
    # --------------------------------------------
    ...
    # --------------------------------------------
        ...
        D_train = DataHumanOpenEnded(
            "train", inc_support_in_query=True, mydir="data_human/open_ended_cross1"
        )
        D_val = DataHumanOpenEnded(
            "val", inc_support_in_query=False, mydir="data_human/open_ended_cross1"
        )
        # D_val = DataHumanOpenEndedIterative('val','data_human/open_ended_cross1')
    elif episode_type == "open_end_human_cross2":
        D_train = DataHumanOpenEnded(
            "train", inc_support_in_query=True, mydir="data_human/open_ended_cross2"
        )
        D_val = DataHumanOpenEnded(
            "val", inc_support_in_query=False, mydir="data_human/open_ended_cross2"
        )
        # D_val = DataHumanOpenEndedIterative('val','data_human/open_ended_cross2')
    elif episode_type == "open_end_human_cross3":
        D_train = DataHumanOpenEnded(
            "train", inc_support_in_query=True, mydir="data_human/open_ended_cross3"
        )
        D_val = DataHumanOpenEnded(
            "val", inc_support_in_query=False, mydir="data_human/open_ended_cross3"
        )
        # D_val = DataHumanOpenEndedIterative('val','data_human/open_ended_cross3')
    elif episode_type == "open_end_human_cross4":
        D_train = DataHumanOpenEnded(
            "train", inc_support_in_query=True, mydir="data_human/open_ended_cross4"
        )
        D_val = DataHumanOpenEnded(
            "val", inc_support_in_query=False, mydir="data_human/open_ended_cross4"
        )
        # D_val = DataHumanOpenEndedIterative('val','data_human/open_ended_cross4')
    elif episode_type == "open_end_human_cross5":
        D_train = DataHumanOpenEnded(
            "train", inc_support_in_query=True, mydir="data_human/open_ended_cross5"
        )
        D_val = DataHumanOpenEnded(
            "val", inc_support_in_query=False, mydir="data_human/open_ended_cross5"
        )
        # D_val = DataHumanOpenEndedIterative('val','data_human/open_ended_cross5')
    elif episode_type == "joint_cross1":
        D_train1 = DataAlgAndBias(
            "train", mydir="data_algebraic", p_noise=0.01, min_ns=14
        )
        D_train2 = DataAlgAndBias(
            "train", mydir="data_algebraic", p_noise=0.01, min_ns=0
        )
        D_train3 = DataHumanOpenEnded(
            "train", inc_support_in_query=True, mydir="data_human/open_ended_cross1"
        )
    ...
    #     D_val = DataAlg('val',mydir='data_algebraic_loose')
        ...
        D_train = DataHumanOpenEnded(
            "train", inc_support_in_query=True, mydir="data_human/legacy_open_ended"
        )
        D_val = DataHumanOpenEnded(
            "val", inc_support_in_query=False, mydir="data_human/legacy_open_ended"
        )
        ...
        D_train = DataAlgAndBias(
            "train", mydir="debug_data_rules++", p_noise=0.01, min_ns=15
        )
        D_val = DataAlg("val", mydir="debug_data_rules++")
        ...
        D_train2 = DataAlgAndBias(
            "train", mydir="debug_data_rules++", p_noise=0.01, min_ns=0
        )
        D_train3 = DataHumanOpenEnded(
            "train", mydir="data_human/legacy_open_ended", inc_support_in_query=True
        )  # like open_end_human
        D_train = MixDataset([D_train1, D_train2, D_train3])
        D_val = DataAlg("val", mydir="debug_data_rules++")
    else:
...
class Lang:
    ...
    def symbols_to_tensor(self, mylist, add_eos=True):
        ...
        output = torch.LongTensor(
            indices
        )  # keep on CPU since this occurs inside Dataset getitem..
        return output
...
def bundle_biml_episode(x_support, y_support, x_query, y_query, myhash, aux={}):
    ...
    x_query_context = [
        item + xy_support for item in x_query
    ]  # Create the combined source sequence for every query
    sample = {}
    sample[
        "identifier"
    ] = myhash  # unique identifying string for this episode (order invariant)
...
def make_biml_batch(samples, langs):
    ...
    mybatch[
        "xq_context"
    ] = []  # list of source sequences (as lists) across all episodes
    ...
    mybatch[
        ...
        []
    )  # bool list indicating whether each query is in its corresponding support set, or not
    ...
    mybatch["xq_context_padded"], mybatch["xq_context_lengths"] = build_padded_tensor(
        mybatch["xq_context"], langs["input"]
    )
    mybatch["yq_padded"], mybatch["yq_lengths"] = build_padded_tensor(
        mybatch["yq"], langs["output"]
    )
    mybatch["yq_sos_padded"], mybatch["yq_sos_lengths"] = build_padded_tensor(
        mybatch["yq"], langs["output"], add_eos=False, add_sos=True
    )
    return mybatch
...
def get_batch_output_pool(batch):
    ...
    if "output_pool" in samples_batch[0]["aux"]:
        ...
        assert all(
            [out_mask_allow == sample["aux"]["output_pool"] for sample in samples_batch]
        ), "The whole batch must have same output pool. Try batch_size=1."
        if EOS_token not in out_mask_allow:
            out_mask_allow.append(EOS_token)
    else:
        assert not any(
            ["output_pool" in sample["aux"] for sample in samples_batch]
        ), "The whole batch must have same output pool. Try batch_size=1."
...
def combine_input_output_symb(list_input_symb, list_output_symb):
    ...
    additional_symb = sorted(
        set([IO_SEP, ITEM_SEP]) - set([EOS_token, SOS_token, PAD_token])
    )
...
class DataAlg(Dataset):
    ...
    def __init__(self, mode, mydir, p_noise=0.0, inc_support_in_query=True, min_ns=0):
...
def add_response_noise(yq_item, p_noise, langs):
    ...
    for i in range(len(yq_item)):  # for each token
        if flip(p_noise):
            ...
            if i == 0:
                new_token = random.choice(choice_set_wo_eos)
...
class DataAlgAndBias(DataAlg):
    ...
    def __init__(
        self, mode, mydir, p_bias=0.2, p_noise=0.0, inc_support_in_query=True, min_ns=0
    ):
        ...
        super().__init__(
            ...
            mydir,
            p_noise=p_noise,
            inc_support_in_query=inc_support_in_query,
            min_ns=min_ns,
        )
        self.p_bias = p_bias
        ...
        for i in range(len(S["xq"])):
            if flip(self.p_bias):  # if true, then make heuristic trial
                if flip():  # one-to-one, left-to-right translation
                    ...
                    S["yq"][i] = use_flipped_grammar(
                        S["xq"][i], S["grammar_str"], self.maxlen
                    )
        if self.p_noise > 0:  # emission noise
            for i in range(len(S["yq"])):
                S["yq"][i] = add_response_noise(S["yq"][i], self.p_noise, self.langs)
...
def use_one2one_heuristic(xq_item, prims, langs):
    ...
    for x_sym in xq_item:
        if x_sym in prims:
            ...
            yq_item.append(
                random.choice(langs["output"].symbols)
            )  # pick from non-special symbols
    return yq_item
...
def use_flipped_grammar(
    xq_item, grammar_str, maxlen, input_symbols=input_symbols_list_default
):
...
class DataHumanProbe(Dataset):
    ...
    def __init__(
        self, mode, data_mult=1, inc_pool=False, mydir="data_human/data_behavior_probes"
    ):
        # Input
        #  mode : train vs. val
        ...
        self.placeholder_length = (
            100000  # placeholder number of episodes (if 'train' mode)
        )
    ...
    def __getitem__(self, idx):
        ...
        if self.do_remap:
            S = assign_random_map(
                S, self.input_symbols, self.output_symbols, output_pool
            )
        if self.inc_support_in_query:
            S["xq"] = S["xs"] + S["xq"]
            S["yq"] = S["ys"] + S["yq"]
...
class DataHumanOpenEnded(Dataset):
    ...
    def __getitem__(self, idx):
        if self.train:
            ...
            S = readfile(
                self.list_items[idx]
            )  ## if we truly want to iterate over files
        assert (
            len(S["xs"]) == 0
        )  # in the text files, all items should be listed as queries
        if self.randomize_order:
...
class MixDataset(Dataset):
    ...
    def __init__(self, list_datasets):
        ...
        for D in self.list_datasets:
            assert (
                self.langs["input"].symbol2index == D.langs["input"].symbol2index
            ), "input libraries must match"
            assert (
                self.langs["output"].symbol2index == D.langs["output"].symbol2index
            ), "output libraries must match"
            D.langs = self.langs
    ...
    def idx_to_D(self, idx):
        ...
        return (
            j,
            new_idx,
        )  # j is index of the right dataset, new_idx is index into that dataset
...
def assign_random_map(S, new_input_symbols, new_output_symbols, output_pool=[]):
    ...
    lang_in_symb_trunc = lang_in_symb[
        : len(unique_in_symb)
    ]  # truncate if all symbols aren't needed
    lang_out_symb_trunc = lang_out_symb[
        : len(unique_out_symb)
    ]  # truncate if all symbols aren't needed
    map_input = lambda command: list_remap(command, unique_in_symb, lang_in_symb_trunc)
    map_output = lambda actions: list_remap(
        actions, unique_out_symb, lang_out_symb_trunc
    )

    # functions to invert mappings (emissions not used in query/support will be mapped to 'undefined_action')
    uo_lhs = lang_out_symb
    uo_rhs = unique_out_symb + ["undefined_action"] * (
        len(uo_lhs) - len(unique_out_symb)
    )
    unmap_input = lambda command: list_remap(
        command, lang_in_symb_trunc, unique_in_symb
    )
    unmap_output = lambda actions: list_remap(actions, uo_lhs, uo_rhs)

    S["aux"] = {}
    if output_pool:
        S["aux"]["output_pool"] = map_output(output_pool)
